...
const Gallery: React.FC = () => {
  ...
  // Optimized Gallery Data Generation
  ...
  /**
   * REFINED SCROLL HANDLER
   * Precision math for responsive scroll tracking
   */
  ...
  /**
   * SMOOTH NAVIGATION
   * Adjusts target scroll based on responsive container heights
   */
  const handleTimelineClick = (index: number) => {
    ...
    const targetScroll = galleryElement.offsetTop + (index / (timelineItems.length - 1)) * galleryHeight;
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-[#0a0a0a] selection:bg-cyan-500/30">
      <div 
      ...
      >
        ...
        {/* RESPONSIVE STICKY HEADER
            Padding adjusts from Mobile (py-2) to Desktop (py-4) 
        */}
        <header className="sticky top-0 z-50 w-full bg-black/60 backdrop-blur-xl border-b border-white/5 py-2 md:py-4 transition-all duration-300">
          <HorizontalTimeline
            items={timelineItems}
        ...
        <main ref={galleryRef} className="w-full flex flex-col items-center pt-4 md:pt-10">
          {gallerySections.map((section) => (
            <section 
              key={section.year} 
              className="w-full max-w-7xl px-4 sm:px-6 md:px-10 mb-12 md:mb-24"
            >
              {/* Responsive Title: fluid font sizing */}
              <motion.h1 
              ...
              */}
              <Masonry columns={{ xs: 2, sm: 3, lg: 4 }} spacing={{ xs: 1.5, sm: 2, md: 3 }}>
                {section.images.map((item) => (
                  ...
                  >
                    <ImageWithSkeleton src={item.url} alt={item.id} height={item.height} />
                  </Box>
                ))}
              </Masonry>
            </section>
        ...
        {/* CAROUSEL SECTION: Independent vertical height for all screens */}
        <section className="relative z-10 w-full min-h-[80vh] md:min-h-screen flex flex-col justify-center items-center bg-gradient-to-t from-black via-black to-transparent py-12 md:py-24 border-t border-white/5">
